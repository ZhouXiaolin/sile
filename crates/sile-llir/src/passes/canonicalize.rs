...
/// MIR lowering does not need to encode that choice permanently.
...
fn inst_operands(op: &InstOp) -> Vec<Operand> {
    match op {
        InstOp::ShapeDim { buf, .. } => vec![buf.clone()],
...
fn remap_inst_op(
    ...
    match op {
        InstOp::ShapeDim { buf, dim } => InstOp::ShapeDim {
            buf: remap_operand(buf, operand_map, result_map),
            dim,
        },
        InstOp::Alloca {
            alloc_ty,
