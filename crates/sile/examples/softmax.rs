...
fn softmax<const BM: i32, const BN: i32>(
    ...
    let tile_x: Tile<f32, { [BM, BN] }> = sile::load_tile_like_2d(x, y);
    let tile_x_max: Tile<f32, { [BM] }> = sile::reduce_max(tile_x.clone(), 1i32);
    ...
    let denom: Tile<f32, { [BM] }> = sile::reduce_sum(num.clone(), 1);
