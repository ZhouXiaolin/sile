...
fn find_live_values(func: &MirFunction) -> HashSet<crate::ValueId> {
    ...
    for block in &func.blocks {
        for inst in &block.insts {
            if is_side_effect(&inst.op) {
                for u in MirFunction::inst_uses(&inst.op) {
                    worklist.push(u);
                }
            }
        }
        for u in MirFunction::terminator_uses(&block.terminator) {
            worklist.push(u);
        }
    }
